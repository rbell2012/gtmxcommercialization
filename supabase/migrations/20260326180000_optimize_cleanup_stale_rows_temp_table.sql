-- cleanup_stale_rows: use temp table + NOT EXISTS (hash-friendly) instead of id != ALL(array),
-- which devolves to O(n×m) on large UUID arrays. Also raise session timeout for large DELETEs.
...
BEGIN
  ...
  IF v_id_type = 'uuid' THEN
    CREATE TEMP TABLE _csr_valid_ids (id uuid) ON COMMIT DROP;
  ...
  ELSE
    CREATE TEMP TABLE _csr_valid_ids (id text) ON COMMIT DROP;
...
COMMENT ON FUNCTION public.cleanup_stale_rows(text, text[]) IS
  'Deletes rows whose id is not in p_valid_ids via temp table + NOT EXISTS; 300s statement_timeout; id uuid or text; sales_teams skips FK-referenced rows.';
