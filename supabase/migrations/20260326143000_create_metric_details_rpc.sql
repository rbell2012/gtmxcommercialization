-- Detail-level monthly metric rollups for names and type breakdowns.
-- This complements get_aggregated_metrics() and keeps accuracy while
-- reducing repeated client-side scans over raw rows.
...
returns table (
  ...
  filtered_wins as (
    select
      w.*,
      coalesce(o.win_stage_date, w.win_date) as effective_date
    from metrics_wins w
