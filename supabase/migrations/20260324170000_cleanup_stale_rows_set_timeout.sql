-- Allow cleanup_stale_rows DELETE to exceed default anon statement_timeout (~8s) on large tables.
...
COMMENT ON FUNCTION public.cleanup_stale_rows(text, text[]) IS
  'Deletes rows whose id is not in p_valid_ids; 60s statement_timeout for large DELETEs; id uuid or text; sales_teams skips FK-referenced rows.';
